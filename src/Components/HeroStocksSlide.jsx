...
export default function HeroStocksSlide({ isLoggedIn }) {
  ...
  return (
    <div className="w-full h-full flex max-md:flex-col justify-center items-center gap-6 md:gap-8 lg:gap-12 my-auto">
      ...
      <div className="z-base w-full md:w-[50%] lg:w-[48%] flex flex-col justify-center text-left order-1 md:order-2 py-1 -mt-2 md:-mt-4">
        {/* Eyebrow Badge */}
        <div className="inline-flex items-center gap-1.5 md:gap-2 px-2.5 md:px-3 py-0.5 md:py-1 rounded-full text-[11px] md:text-xs font-semibold bg-[#2edbad]/10 text-[#2edbad] border border-[#2edbad]/30 w-fit mb-4 md:mb-6">
        ...
        <p className="text-xs sm:text-sm md:text-base text-[#b2b8c2] mt-4 md:mt-6 text-left leading-relaxed max-w-xl">
      ...
      <div className="flex justify-center items-center w-full md:w-[50%] lg:w-[52%] order-2 md:order-1 -mt-2 md:-mt-4">
        <video
          ref={videoRef}
          src="/hero_markets_video_v4_intro.mp4"
          className="w-full max-w-full sm:max-w-lg md:max-w-[560px] lg:max-w-[620px] xl:max-w-[660px] max-h-[320px] sm:max-h-[380px] md:max-h-[480px] lg:max-h-[520px] object-contain rounded-2xl mix-blend-screen"
          autoPlay
          loop
          muted
          playsInline
      ...
      <div className="flex items-center gap-3 mt-3 justify-between w-full md:hidden order-3">
        <Button
          variant="primary"
      ...
      <div className="flex gap-2.5 md:hidden mt-2.5 order-4">
        <a
          href="https://apps.apple.com/app/bitzup/id6749609757"
          target="_blank"
          rel="noreferrer"
